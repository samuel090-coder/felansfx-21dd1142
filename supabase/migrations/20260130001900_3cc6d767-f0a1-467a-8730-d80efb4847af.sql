-- Fix the settle_binary_position function to ensure proper numeric handling
-- and add explicit logging for debugging
...
BEGIN
  ...
  -- Calculate P&L with explicit casting to ensure precision
  ...
  -- Credit amount: if win, return stake + profit; if loss, nothing
  ...
  -- Update the appropriate wallet based on account type
  IF COALESCE(v_pos.account_type, 'demo') = 'demo' THEN
    ...
    END IF;
  ELSE
    -- Real wallet update - ALWAYS update when it's a win
    IF v_is_win THEN
      ...
      -- Return detailed info including balance change
