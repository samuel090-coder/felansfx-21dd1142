                ...
                {activity.map((item) => (
                  <div key={item.title} className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-white/55">{item.meta}</p>
                    </div>
                    {item.amount ? <p className={cn('text-[11px] font-semibold', item.tone === 'success' ? 'text-success' : 'text-destructive')}>{item.amount}</p> : <ArrowUpRight className="h-3.5 w-3.5 shrink-0 text-white/36" />}
                  </div>
                ))}
              </div>
            </FintechCard>
